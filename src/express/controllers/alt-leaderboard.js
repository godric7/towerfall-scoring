...
const altLeaderBoardCtrl = (req /*: Request */, res /*: Response */) => {
  ...
  const countAllGamesPerPlayer /*: { [Player]: number } */ = [].concat.apply([],
    games
    .map(game => game.rankings)
    .map(rankings => Object.keys(rankings))
  ).reduce(reduceCount, {});

  const firstRankPerPlayer /*: { [Player]: number } */ = games
    .map(game => game.rankings)
    .map(rankings => Object.keys(rankings)
      .find(player => rankings[player] == 1))
    .reduce(reduceCount, {});
...
const reduceCount = (mergeObject /*: any */, key /*: ?Player */) /*: { [Player]: number } */ => {
